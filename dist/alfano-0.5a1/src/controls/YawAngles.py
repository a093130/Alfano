...
#            level=logging.INFO,
    ...
    AOL = np.linspace(0, 360, 60) # Initialize Argument Of Latitude (AOL).
    mpl.rcParams['legend.fontsize'] = 10
    fig, axs = plt.subplots(2,1)

    """ Test Case 0: initialize dictionary structure from file."""
    ...
    """ Test Case 3: Log thrust Components per Revolution."""
    SMA_init = 6838.1366    # Semi-Major Axis
    ...
    costate = -0.52         # Alfano lambda
    """@TODO this function would be really valuable if the costate were user selected (ComboBox)."""
    thrustv = [0, 1.0, 0]   # thrust vector, initialized
        
    with open('thrustlog.log', 'w+') as log:
        log.write('Test Case 3\n Orbit Ratios from 1 to 10')
        log.write('For lambda = {0}.\n'.format(costate))
        log.write('By Argument of Latitude for select R\n')
        ...
        for R in range(R_init, R_final, 1):
            ...
            log.write('\nOrbit SMA: {0}\n'.format(SMA)) 
            for long in AOL:                
                thrustv = get_control_onrev(costate, long, SMA)
                
                log.write('AOL = {0:.6g},\
                     velocity: {1:.2g}, normal: {2:.2g}, binormal: {3:.2g}'.format(long, thrustv[0], thrustv[1], thrustv[2]))
